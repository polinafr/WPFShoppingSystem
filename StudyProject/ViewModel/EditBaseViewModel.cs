...
namespace StudyProject.ViewModel
{
    public class EditBaseViewModel:BaseViewModel
    {
        ...
        private void InitData()//info initialisation from DB when program starts
        {

                StoredList = MainViewModel.DALimp.getAllStore();
                GoodTypeList = MainViewModel.DALimp.getAllType();
                AddGood =new Model.Good();
                _ = DownloadGoods(); // thread for DownloadGoods func
        ...
        private async Task DownloadGoods()//async get list of goods and pictures from firebase
        {
            await Task.Run(() => {
